...
if ! grep -q "meta-navqplus-apt-ros" conf/bblayers.conf; then
	# Add navq+ BSP meta layer
	echo "BBLAYERS += \"\${BSPDIR}/sources/meta-navqplus-apt-ros\"" >> conf/bblayers.conf
	...
	linux_rev=$(repo forall meta-navqplus-apt-ros -c 'echo $REPO__linux_rev')
	[ -n "$linux_rev" ] && echo "SRCREV:pn-linux-imx = \"$linux_rev\"" >> conf/site.conf

	# check if u-boot revision must be bound
	uboot_rev=$(repo forall meta-navqplus-apt-ros -c 'echo $REPO__uboot_rev')
